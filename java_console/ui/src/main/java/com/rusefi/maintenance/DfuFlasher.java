...
public class DfuFlasher {
    ...
    public static void doAutoDfu(JComponent parent, @NotNull String port, UpdateOperationCallbacks callbacks) {
        ...
        if (!ProgramSelector.IS_WIN) {
            ...
            callbacks.log("ECU-tech console can only program on Windows");
            return;
        }
    ...
    private static void executeDFU(UpdateOperationCallbacks callbacks, boolean fullErase) {
        ...
        if (!driverIsHappy) {
            callbacks.log("*** DRIVER ERROR? *** Did you have a chance to try 'Install Drivers' button on top of ECU-tech console start screen?");
            callbacks.error();
            return;
        }
        ...
        if (stdout.toString().contains("Download verified successfully")) {
            ...
            callbacks.log("ERROR: Please try installing drivers using 'Install Drivers' button on ECU-tech splash screen");
...
//        System.out.println("DFU lookup cost " + cost + "ms");
    ...
    private static String getDfuWriteCommand() throws FileNotFoundException {
        String fileName = Launcher.INPUT_FILES_PATH + File.separator + "ecu.bin";
        if (fileName == null)
            throw new FileNotFoundException("File not found: ecu.bin");
    ...
    static List<String> getDevicesReport() {
        ...
        try {
            ...
            while ((line = stdout.readLine()) != null) {
                ...
                if (!lowerCase.contains("stm32") && !lowerCase.contains("dfu") && !lowerCase.contains("ecu"))
                    continue;
                report.add(line);
            }
            stdout.close();
