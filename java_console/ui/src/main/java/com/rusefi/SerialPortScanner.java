...
public enum SerialPortScanner {
    ...
    public enum SerialPortType {
        FomeEcu("ECU-tech ECU", 20),
        FomeEcuWithOpenblt("ECU-tech ECU w/ BL", 20),
    ...
    private static PortResult inspectPort(String serialPort) {
        ...
        if (isOpenblt) {
            ...
            boolean isEcu = signature != null && signature.contains("ECU-tech");
            log.info("Port " + serialPort + (isEcu ? " looks like" : " does not look like") + " a ECU-tech ECU");
            if (isEcu) {
                boolean ecuHasOpenblt = ecuEcuHasOpenblt(serialPort);
                log.info("ECU-tech ECU at " + serialPort + (ecuHasOpenblt ? " has" : " does not have") + " an OpenBLT bootloader");
    ...
    public static boolean ecuEcuHasOpenblt(String port) {
        try (IoStream stream = BufferedSerialIoStream.openPort(port)) {
            if (stream == null) {
                return false;
            }
            ...
            byte[] response = stream.getDataBuffer().getPacket(500, "ecuEcuHasOpenblt");
