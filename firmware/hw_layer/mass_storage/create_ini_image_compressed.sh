...
# create a FAT filesystem inside, name it "ECU-tech EFI"
mkfs.fat $IMAGE
# labels can be no longer than 11 characters
fatlabel $IMAGE "ECU-tech EFI"
...
mcopy -i $IMAGE hw_layer/mass_storage/filesystem_contents/ECU-tech\ Wiki.url ::
mcopy -i $IMAGE hw_layer/mass_storage/filesystem_contents/ECU-tech\ Discord.url ::
mcopy -i $IMAGE hw_layer/mass_storage/filesystem_contents/ECU-tech\ Releases.url ::
