...
cp java_console_binary/ecu_console.jar $CONSOLE_FOLDER
cp simulator/build/ecu_simulator.exe     $CONSOLE_FOLDER
cp misc/console_launcher/ecu_*.exe     $CONSOLE_FOLDER
...
cp firmware/deliver/ecu.bin $FOLDER

# bootloader
[ -e firmware/deliver/ecu_bl.bin ] && { cp firmware/deliver/ecu_bl.bin $FOLDER ; }
# update srec
[ -e firmware/deliver/ecu_update.srec ] && { cp firmware/deliver/ecu_update.srec $FOLDER ; }

[ -e firmware/deliver/ecu.bin ] || { echo "$SCRIPT_NAME: ecu.bin not found"; exit 1; }
...
rm -rf $CONSOLE_FOLDER/ecu_console.exe
